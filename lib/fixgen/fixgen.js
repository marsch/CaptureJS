...
var fixgen = function (options) {
	...
	**/
	...
	that.onHttpRequestBody = function (session, http, data) { 
		if (!that.matchesFilter(http)) return;
		if(typeof http.request.body === "undefined") {
			http.request.body = "";
		}
	...
	that.gzipDecode = function (buf) {  
		try {
		var gunzip = new compress.Gunzip;
		gunzip.init();  
		var decoded = gunzip.inflate(buf, "binary");
		var end = gunzip.end();
	...
	that.onHttpResponseBody = function (session, http, data) { 
		if (!that.matchesFilter(http)) return;
		if (/gzip/.test(http.response.headers["Content-Encoding"])) {
			//http.response.headers["Content-Endoding"] = null;
			http.response.body +=  data.toString("binary");//that.gzipDecode(data);
		} else {
	...
	that.onHttpResponseComplete = function (session, http) { 
		if (!that.matchesFilter(http)) return true; 
